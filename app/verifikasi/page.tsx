...
export default function VerifikasiPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch('/api/measurements?status=all', { credentials: 'include' });
      if (res.ok) {
  ...
  useEffect(() => {
    // Attempt to get user info if endpoint exists, otherwise we'll just try
  ...
  const handleDelete = async (trackingId: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error("Only admins can perform this action or delete failed.");
      }
      fetchData();
    } catch (err: any) {
      alert(err.message);
            ...
            <Card key={item.id} className="glass-card p-4 flex items-center justify-between">
              <div>
                ...
                <div className="flex gap-4 mt-1 text-sm font-semibold">
                  <span>AI Pred: <span className={item.ai_pred_class === 'OSCC' ? 'text-red-500' : 'text-emerald-500'}>{item.ai_pred_class || 'N/A'}</span></span>
                  <span>Prob: {item.ai_probability ? Number(item.ai_probability).toFixed(1) + '%' : 'N/A'}</span>
                </div>
              </div>
              <div className="flex flex-col gap-2">
                <Link
                  href={`/verify/${item.tracking_id}`}
                  className="px-4 py-2 bg-amber-500 hover:bg-amber-600 text-center text-white rounded-lg text-sm font-semibold transition-colors duration-200"
                >
                  Verifikasi
                </Link>
                {/* We just render the button if role is admin or unknown (let it fail) */}
                <button
                  onClick={() => handleDelete(item.tracking_id)}
                  className="px-4 py-2 bg-red-500 hover:bg-red-600 text-center text-white rounded-lg text-sm font-semibold transition-colors duration-200"
                >
                  Hapus
                </button>
              </div>
